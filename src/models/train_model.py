...
class models_methods():
    def first_method(model, params, X_train, y_train, X_test, y_test):
        """Optimize models usin only cross validation."""
        model_opt = GridSearchCV(model, params ,cv = 5)
        model_opt.fit(X_train,y_train)
        y_pred = model_opt.predict(X_test)
        confusion = confusion_matrix(y_test, model_opt.predict(X_test))
        plt.figure(figsize=(8,8))
        sns.heatmap(confusion, annot=True)
        plt.xlabel("Predicted")
        plt.ylabel("Actual")
        print(classification_report(y_test, y_pred))
        print(f'\nBest Parameters of this model are : {model_opt.best_params_}\n')

    def second_method(pipe, params, XX_train, YY_train):
